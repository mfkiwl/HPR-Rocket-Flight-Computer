...
//---------------------------------
  ...
  //SDIO Setup: v2.X now works now that the RadioHead ISR problem is fixed
  SdFs SD;
  FsFile outputFile;
  FsFile settingsFile;
...
void beginSD(){
  ...
  //Teensy 3.2 or 4.0
    ...
    if(pins.SD_CS == pins.nullFire){
      ...
      else{Serial.println(F("SD Card OK!"));}}//BUILTIN_SDCARD
...
void writeSDflightData(){
  ...
  writeIntData(gyro.z);
  //HighG Accel Data
  if(threeAxisMode){writeIntData(highG.x);writeIntData(highG.y);}
...
void writeSDfooter(){
  ...
  writeFloatData(baroPressOffset,2);
  writeFloatData(baroTempOffset,1);
